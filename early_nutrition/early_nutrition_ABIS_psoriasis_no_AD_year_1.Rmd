---
title: "ABIS-psoriasis year-1 excluding auto-immune diseases"
...
if (curr_os == "Windows"){
  output_dir <- "C:\\Users\\debda22\\OneDrive - Linköpings universitet\\early_nutrition\\without_auto_immune_disease\\revision_1_year"
  # output_dir <- "C:\\Users\\debda22\\OneDrive - Linköpings universitet\\early_nutrition\\without_auto_immune_disease\\ABIS_year_1_final"
} else {
  output_dir <- "/Users/debojyoti/OneDrive - Linköpings universitet/early_nutrition/without_auto_immune_disease/revision_1_year"
  # output_dir <- "/Users/debojyoti/OneDrive - Linköpings universitet/early_nutrition/without_auto_immune_disease/ABIS_year_1_final"
}
...
source("all_parameters_ABIS_year_1.R", local = knitr::knit_global())
source("C:/Users/debda22/OneDrive - Linköpings universitet/Rmd_and_R_files/old_scripts/useful_functions_ABIS_psoriasis_no_AD.R", local = knitr::knit_global())
...
# abis_table <- read_sav("C:\\Users\\debda22\\Documents\\Linkoping_projects\\ABIS_RAW_DATA_SPSS\\Variables.sav")
# cohort_table <- read_sav("C:\\Users\\debda22\\Documents\\Linkoping_projects\\ABIS_RAW_DATA_SPSS\\Early_nutrition_230223.sav")
# psoriasis_diagonistic_table <- read_sav("C:\\Users\\debda22\\Documents\\Linkoping_projects\\ABIS_RAW_DATA_SPSS\\L40.sav")
...
columns of interest for 1st year
```{r}
columns_of_interest <- readxl::read_excel(
  "C:\\Users\\debda22\\Documents\\Linkoping_projects\\ABIS_RAW_DATA_SPSS\\early_nutrition_1_and_3_years.xlsx", 
  sheet = "1 year", 
  col_names = FALSE
  )
...
columns_of_interest <- rbind(columns_of_interest, c("AutoimmDisease", 
                                                    "AutoimmuneDisease"))
columns_of_interest <- rbind(
  columns_of_interest, c("child_dob", "Birth of children year and month")
  )
...
explanatory_vars <- c(
  ...
  "b_45",
  "b_46",
  # "b_47", no categorization possible
  "b_49",
  "b_53",
  "b_54",
  "b_55",
...
############ only cases where Diagnosis was done twice
```{r}
psoriasis_data_full <- psoriasis_data_full %>%  
  mutate(Diagnos_twice = case_when(                              
      (!is.na(detection_date_diff))&(detection_date_diff != 0)&(Diagnos == 1) ~ 1,  ## recode L40 (psoriasis diagnosis) to 1
      Diagnos == 0 ~ 0,       ## not present  to 0
      TRUE ~ NA_real_)        ## otherwise set to missing
    )
  
psoriasis_data_full$Diagnos <- psoriasis_data_full$Diagnos_twice

psoriasis_data_full$Diagnos_twice <- NULL
...
```{r}
all_parameters[all_parameters$category_variable == "pork",]$labels[[1]] <- c("3-5 times/week - daily","1-2 times/week", "seldom")
all_parameters[all_parameters$category_variable == "pork",]$breaks[[1]] <- c(0,2,3,4)
...
```{r}
multivar_data <- psoriasis_data_full[,c("ABISnr", "b_5", "b_6",
                                              "b_10", "Gender", "Diagnos")]

multivar_explanatory_vars <- c("b_5", "b_6","b_10", "Gender")

multivar_explanatory_vars_cats <- c("exclusive_breastfeeding_dichotomous_4", 
                                    "infant_formula_dichotomous_4", 
                                    "total_breastfeeding_dichotomous_4", 
                                    "Gender_cat")
...
```{r}
# plot_data <- psoriasis_data_full[,c("ABISnr", 
#                                     "b_5", 
#                                     "Diagnos")]
# 
# p1 <- cumulative_percentage(psoriasis_data_full, "Diagnos", "b_5", 
#                             xlabel = "exclusive breastfeeding", inverse = TRUE)
# p2 <- cumulative_percentage(psoriasis_data_full, "Diagnos", "b_6", 
#                             xlabel = "infant formula", inverse = FALSE)
# p3 <- cumulative_percentage(psoriasis_data_full, "Diagnos", "b_10", 
#                             xlabel = "total breastfeeding", inverse = TRUE)
# p4 <- cumulative_percentage(psoriasis_data_full, "Diagnos", "b_9", 
#                             xlabel = "gluten", inverse = FALSE)
...
```{r}
multivar_data <- psoriasis_data_full[,c("ABISnr", "b_5", "Gender", "Diagnos")]

multivar_explanatory_vars <- c("b_5", "Gender")
...
final_data_exclusive_bf <- multivariable_regression_table(
  multivar_data,
  multivar_explanatory_vars_cats)

final_data_exclusive_bf <- final_data_exclusive_bf %>% 
  add_header_row(
    values = c("Multivariable regression: All genders but without other AD"), 
    colwidths = 6)
```
...
```{r}
multivar_data <- psoriasis_data_full[,c("ABISnr", "b_6","b_10", 
                                        "Gender", "Diagnos")]

multivar_explanatory_vars <- c("b_6","b_10", "Gender")

multivar_explanatory_vars_cats <- c("infant_formula_dichotomous_4", 
                                    "total_breastfeeding_dichotomous_4", 
                                    "Gender_cat")
...
final_data_wo_exclusive_bf <- multivariable_regression_table(
  multivar_data,
  multivar_explanatory_vars_cats)

final_data_wo_exclusive_bf <- final_data_wo_exclusive_bf %>% 
  add_header_row(
    values = c("Multivariable regression: All genders but without other AD"), 
    colwidths = 6)
```
...
```{r}
# pdf_file <- paste(
#   output_dir,"ABIS_1_year_early_nutrition_psoriasis_cumulative_percentage.pdf", 
#   sep = "/")
# 
# pdf(pdf_file)
# 
...
# 
# jpeg_file <- paste(
#   output_dir,"ABIS_1_year_early_nutrition_psoriasis_cumulative_percentage.jpeg", 
#   sep = "/")
# 
# jpeg(jpeg_file)
# 
...
pdf(
  paste(
    output_dir,
    "chi_sq_test_and_percentage_breakdown_early_nutrition_year_1_multivarible.pdf",
  sep = "/"))
...
flextable::save_as_html(
  final_univ_tab_tidy, 
  path = paste(output_dir,"univariate_table_year_1_ABIS.html", 
               sep = "/")
  )
flextable::save_as_docx(
  final_univ_tab_tidy, 
  path = paste(output_dir,"univariate_table_year_1_ABIS.docx", 
               sep = "/")
  )


flextable::save_as_docx(
  final_data_exclusive_bf, 
  path = paste(output_dir,
               "multivariable_table_year_1_ABIS_exclusive_bf.docx",
               sep = "/")
  )
flextable::save_as_html(
  final_data_exclusive_bf, 
  path = paste(output_dir,
               "multivariable_table_year_1_ABIS_exclusive_bf.html",
               sep = "/")
  )

flextable::save_as_docx(
  final_data_wo_exclusive_bf, 
  path = paste(output_dir,
               "multivariable_table_year_1_ABIS_wo_exclusive_bf.docx", 
               sep = "/")
  )
flextable::save_as_html(
  final_data_wo_exclusive_bf, 
  path = paste(output_dir,
               "multivariable_table_year_1_ABIS_wo_exclusive_bf.html", 
               sep = "/")
  )
...
# age >= puberty age
```{r}
all_parameters[all_parameters$category_variable == "fish_other",]$labels[[1]] <- c("daily","3-5/1-2 times/week","seldom")
all_parameters[all_parameters$category_variable == "fish_other",]$breaks[[1]] <- c(0,1,3,4)

all_parameters[all_parameters$category_variable == "beef",]$labels[[1]] <- c("daily","3-5 times/week","1-2 times/week seldom")
all_parameters[all_parameters$category_variable == "beef",]$breaks[[1]] <- c(0,1,2,4)
...
```{r}
multivar_data <- psoriasis_data_gt_puberty[,c("ABISnr","b_10","b_46", 
                                             "Gender","Diagnos")]

multivar_explanatory_vars <- c("b_10","b_46", "Gender")

multivar_explanatory_vars_cats <- c("total_breastfeeding_dichotomous_4",
                                    "other_vitamin",
                                    "Gender_cat")
...
# age < puberty age
```{r}
all_parameters[all_parameters$category_variable == "fish_other",]$labels[[1]] <- c("1-2/3-5 times/week/daily","seldom")
all_parameters[all_parameters$category_variable == "fish_other",]$breaks[[1]] <- c(0,3,4)
...
```{r}
multivar_data <- psoriasis_data_lt_puberty[,c("ABISnr","b_5", 
                                              "Gender","Diagnos")]
...
multivariable_regression_lt_puberty_exclusive_bf <- multivariable_regression_table(
  multivar_data,
  multivar_explanatory_vars_cats)

multivariable_regression_lt_puberty_exclusive_bf <- 
  multivariable_regression_lt_puberty_exclusive_bf %>% 
  add_header_row(
...
```{r}
multivar_data <- psoriasis_data_lt_puberty[,c("ABISnr", 
                                              "b_6", 
                                              "b_10", 
                                              "b_58",
                                              "Gender",
...
multivar_explanatory_vars <- c("b_6", "b_10", "b_58", "Gender")
...
multivariable_regression_lt_puberty_wo_exclusive_bf <- multivariable_regression_table(
  multivar_data,
  multivar_explanatory_vars_cats)

multivariable_regression_lt_puberty_wo_exclusive_bf <- 
  multivariable_regression_lt_puberty_wo_exclusive_bf %>% 
  add_header_row(
    values = c(paste("Multivariable regression: Detected with psoriasis before",
               as.character(puberty_age)," years")), 
...
flextable::save_as_docx(
  univariate_table, 
  path = paste(output_dir,"univariate_table_gt_puberty_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  univariate_table, 
  path = paste(output_dir,"univariate_table_gt_puberty_year_1_ABIS.html", sep = "/")
  )
...
flextable::save_as_docx(
  multivariate_table,
  path = paste(output_dir,"multivariate_table_gt_puberty_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  multivariate_table,
  path = paste(output_dir,"multivariate_table_gt_puberty_year_1_ABIS.html", sep = "/")
  )
...
flextable::save_as_docx(
  univariate_table, 
  path = paste(output_dir,"univariate_table_lt_puberty_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  univariate_table, 
  path = paste(output_dir,"univariate_table_lt_puberty_year_1_ABIS.html", sep = "/")
  )



multivariate_table <- multivariable_regression_lt_puberty_exclusive_bf

flextable::save_as_docx(
  multivariate_table, 
  path = paste(output_dir,
               "multivariate_table_lt_puberty_year_1_ABIS_exclusive_bf.docx",
               sep = "/")
  )
flextable::save_as_html(
  multivariate_table, 
  path = paste(output_dir,
               "multivariate_table_lt_puberty_year_1_ABIS_exclusive_bf.html", 
               sep = "/")
  )

multivariate_table <- multivariable_regression_lt_puberty_wo_exclusive_bf

flextable::save_as_docx(
  multivariate_table, 
  path = paste(output_dir,
               "multivariate_table_lt_puberty_year_1_ABIS_wo_exclusive_bf.docx", 
               sep = "/")
  )
flextable::save_as_html(
  multivariate_table, 
  path = paste(output_dir,
               "multivariate_table_lt_puberty_year_1_ABIS_wo_exclusive_bf.html", 
               sep = "/")
  )
...
# boys only
```{r}
all_parameters[all_parameters$variable2categorise == "b_55",]$labels[[1]] <- c("3-5 times/week - daily", "1-2 times/week - seldom")
all_parameters[all_parameters$variable2categorise == "b_55",]$breaks[[1]] <- c(0,2,4)
...
## for boys only exclusive breastfeeding (4 months) was found to be significant in univariate regression
...
# girls only
```{r}
all_parameters[all_parameters$category_variable == "beef",]$labels[[1]] <- c("3-5 times/week - daily", "1-2 times/week - seldom")
all_parameters[all_parameters$category_variable == "beef",]$breaks[[1]] <- c(0,2,4)


all_parameters[all_parameters$category_variable == "pork",]$labels[[1]] <- c("3-5 times/week - daily", "1-2 times/week - seldom")
all_parameters[all_parameters$category_variable == "pork",]$breaks[[1]] <- c(0,2,4)
...
```{r}
multivar_data <- psoriasis_data_girls[,c("ABISnr","b_6", "b_10", 
                                         "b_46","Diagnos")]

multivar_explanatory_vars <- c("b_6", "b_10", "b_46")
...
multivariable_regression_girls <- multivariable_regression_table(
  multivar_data,
  multivar_explanatory_vars_cats)

multivariable_regression_girls <- multivariable_regression_girls %>%
  add_header_row(
    values = c("Multivariable regression: Detected with psoriasis in girls"),
    colwidths = 6)
```
...
flextable::save_as_docx(
  univariate_table, 
  path = paste(output_dir,"univariate_table_boys_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  univariate_table, 
  path = paste(output_dir,"univariate_table_boys_year_1_ABIS.html", sep = "/")
  )
...
flextable::save_as_docx(
  univariate_table, 
  path = paste(output_dir,"univariate_table_girls_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  univariate_table, 
  path = paste(output_dir,"univariate_table_girls_year_1_ABIS.html", sep = "/")
  )

multivariate_table <- multivariable_regression_girls

flextable::save_as_docx(
  multivariate_table,
  path = paste(output_dir,"multivariate_table_girls_year_1_ABIS.docx", sep = "/")
  )
flextable::save_as_html(
  multivariate_table,
  path = paste(output_dir,"multivariate_table_girls_year_1_ABIS.html", sep = "/")
  )
```
